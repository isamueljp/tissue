...
const Sidebar = () => {
  ...
  // Premium Fourth Degree Logo
  const FourthDegreeLogo = () => (
      ...
      <nav className="space-y-2">
        {menuItems.map(item => (
          <Link 
            key={item.path} 
            to={item.path} 
            className={`sidebar-item ${location.pathname === item.path ? 'bg-[#00197e]/20 text-[#00197e]' : 'text-gray-400 hover:text-white'}`}
          >
